...
def outputDistortion(baselinePoses, poses):
  ...
  return peakMax, distanceAcc

def getBoxplotData(pathToBaseline, pathToOutputFiles):
  fixedData = getData(pathToBaseline)
  dataAll = getData(pathToOutputFiles)
  distortions = []
  for data in dataAll:
    distortionMax, distortion = outputDistortion(fixedData[12], data)
...
def getThreshold(pathToOutputFiles):
  ...
  for i in range(lenDataAll):
    for j in range((i + 1), lenDataAll):
      distortionMax, distortion = outputDistortion(dataAll[i], dataAll[j])
      distortions.append(distortionMax)
...
pathToBaseline = sys.argv[1]
pathToOutputFiles = sys.argv[2]
getBoxplotData(pathToBaseline, pathToOutputFiles)
